...
void bhv_platform_on_track_init(void)
{
    ...
    {
        s16 pathIndex =
            (u16)(o->oBehParams >> 16) & PLATFORM_ON_TRACK_BP_MASK_PATH;
        o->oPlatformOnTrackType =
            ((u16)(o->oBehParams >> 16) & PLATFORM_ON_TRACK_BP_MASK_TYPE) >> 4;

        o->oPlatformOnTrackIsNotSkiLift =
            o->oPlatformOnTrackType - PLATFORM_ON_TRACK_TYPE_SKI_LIFT;

        o->collisionData = segmented_to_virtual(
            sPlatformOnTrackCollisionModels[o->oPlatformOnTrackType]);

        o->oPlatformOnTrackStartWaypoint = segmented_to_virtual(
            sPlatformOnTrackPaths[pathIndex]);
...
static void platform_on_track_act_init(void)
{
    ...
    o->oForwardVel = o->oVelX = o->oVelY = o->oVelZ =
        o->oPlatformOnTrackDistMovedSinceLastBall = 0.0f;
...
static void platform_on_track_act_move_along_track(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (approach_f32_ptr(
                &o->oPlatformOnTrackDistMovedSinceLastBall,
                300.0f,
                o->oForwardVel))
            {
                ...
                platform_on_track_update_pos_or_spawn_ball(
                    5, o->oHomeX, o->oHomeY, o->oHomeZ);
            }
        }

        platform_on_track_update_pos_or_spawn_ball(
            0, o->oPosX, o->oPosY, o->oPosZ);
...
void bhv_track_ball_update(void)
{
    ...
    s16 relativeIndex = (s16)o->oBehParams2ndByte -
        (s16)o->parentObj->oPlatformOnTrackBaseBallIndex - 1;
