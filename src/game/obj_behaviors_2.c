...
static s16 obj_turn_pitch_toward_mario(f32 targetOffsetY, s16 turnAmount)
{
    ...
    targetPitch = obj_turn_toward_object(
        o, gMarioObject, O_MOVE_ANGLE_PITCH_INDEX, turnAmount);
    o->oPosY += targetOffsetY;
    ...
    *value += (s32)*vel;
    ...
    if (*value == target ||
        ((*value - target) * (startValue - target) < 0 &&
            *vel > -velCloseToZero && *vel < velCloseToZero))
    {
        *value = target;
        *vel = 0.0f;
        return TRUE;
...
static void obj_update_blinking(
...
static void obj_act_squished(f32 baseScale)
{
    ...
    if (approach_f32_ptr(&o->header.gfx.scale[1], targetScaleY, baseScale * 0.14f))
    {
        o->header.gfx.scale[0] = o->header.gfx.scale[2] =
            baseScale * 2.0f - o->header.gfx.scale[1];
...
void BehSmallPiranhaFlameLoop(void)
{
    ...
    else
    {
        ...
        obj_move_standard(-78);
        spawn_object_with_scale(o, o->oSmallPiranhaFlameUnkFC, bhvSmallPiranhaFlame, 0.4f * o->header.gfx.scale[0]);

        if (o->oTimer > o->oSmallPiranhaFlameUnk100)
        {
            spawn_object_relative_with_scale(1, 0, o->oGraphYOffset, 0, o->header.gfx.scale[0], o, o->oSmallPiranhaFlameUnkFC, bhvFlyguyFlame);
    ...
    if (o->oAction == 0)
    {
        if (!(o->activeFlags & 0x0008))
        {
            for (val08 = 0; val08 < 6; val08++)
            {
                ...
                val0C = spawn_object_relative(val08 & 0x00000001, D_80331C00[val08].unk00, 0, val06, o, MODEL_BBH_WOODEN_TOMB, bhvHauntedRoomCheckSubobject);
                if (val0C != NULL)
                {
                    if (val06 > 0)
                    {
