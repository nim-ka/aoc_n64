...
s16 set_mario_animation(struct MarioState *m, s32 targetAnimID)
{
    ...
    {
        targetAnim->values =
            (void *) VIRTUAL_TO_PHYSICAL((s8 *) targetAnim + (s32) targetAnim->values);
        targetAnim->index =
            (void *) VIRTUAL_TO_PHYSICAL((s8 *) targetAnim + (s32) targetAnim->index);
    }
...
s16 set_mario_anim_with_accel(struct MarioState *m, s32 targetAnimID, s32 accel)
{
    ...
    if (o->header.gfx.unk38.animID != targetAnimID)
    {
        ...
        else
        {
            if (targetAnim->flags & ANIM_FLAG_FORWARD)
            {
                o->header.gfx.unk38.animFrameAccelAssist =
                    (targetAnim->unk04 << 0x10) + accel;
            }
            else
            {
                o->header.gfx.unk38.animFrameAccelAssist =
        ...
        o->header.gfx.unk38.animFrame = (
            o->header.gfx.unk38.animFrameAccelAssist >> 0x10);
    }
...
s32 is_anim_past_frame(struct MarioState *m, s16 animFrame)
{
    ...
    if (animInfo->animAccel)
    {
        if (curAnim->flags & ANIM_FLAG_FORWARD)
        {
            isPastFrame = (animInfo->animFrameAccelAssist > acceleratedFrame) &&
                    (acceleratedFrame >=
                    (animInfo->animFrameAccelAssist - animInfo->animAccel));
        }
        else
        {
            isPastFrame = (animInfo->animFrameAccelAssist < acceleratedFrame) &&
...
void update_mario_pos_for_anim(struct MarioState *m)
{
    ...
    flags = find_mario_anim_flags_and_translation(
                m->marioObj, m->faceAngle[1], translation
            );
...
void play_sound_and_spawn_particles(struct MarioState *m, u32 soundBits, u32 waveParticleType)
{
    ...
    if (
        ...
        soundBits == SOUND_ACTION_UNKNOWN443 ||
...
s32 mario_get_floor_class(struct MarioState *m)
{
    ...
    if (
        m->action == ACT_CRAWLING && m->floor->normal.y > 0.5f &&
        floorClass == SURFACE_CLASS_DEFAULT
    )
    {
        floorClass = SURFACE_CLASS_NOT_SLIPPERY;
...
u32 mario_floor_is_slippery(struct MarioState *m)
{
    ...
    if (
        ...
        TERRAIN_SLIDE && m->floor->normal.y < 0.9998477f //~cos(1 deg)
        )
    {
        return TRUE;
    }
...
void debug_print_speed_action_normal(struct MarioState *m)
{
    ...
    if (gShowDebugText)
    {
        ...
        print_text_fmt_int(210, 88, "ANG %d",
            (atan2s(floor_nY, steepness) * 180.0f) / 32768.0f
        );
...
void set_submerged_cam_preset_and_spawn_bubbles(struct MarioState *m)
{
    ...
    if ((m->action & ACT_GROUP_MASK) == ACT_GROUP_SUBMERGED)
    {
        ...
        {
            if ((heightBelowWater > 800.0f) &&
                (camPreset != CAMERA_PRESET_BEHIND_MARIO)
            )
            {
                func_80285BD8(m->area->camera, CAMERA_PRESET_BEHIND_MARIO, 1);
            }

            if ((heightBelowWater < 400.0f) &&
                (camPreset != CAMERA_PRESET_WATER_SURFACE)
            )
            {
                func_80285BD8(m->area->camera, CAMERA_PRESET_WATER_SURFACE, 1);
            }
...
u32 update_and_return_cap_flags(struct MarioState *m)
{
    ...
    if (m->capTimer > 0)
    {
        ...
        if (m->capTimer == 0)
        {
            ...
            if ((m->flags &
                (MARIO_NORMAL_CAP | MARIO_VANISH_CAP |
