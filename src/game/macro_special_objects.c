...
/*
 * Spawns an object at an absolute location with currently 3 unknown variables that get converted to floats.
    ...
    for (i = 0; i < numOfSpecialObjects; i++)
    {
        ...
        switch (type)
        {
            ...
            case SPTYPE_PARAMS_AND_YROT:
                ...
                break;
            case SPTYPE_UNKNOWN:
                extraParams[0] = **specialObjList; // Unknown, gets put into obj->oUnknownUnk108_F32 as a float
                (*specialObjList)++;
