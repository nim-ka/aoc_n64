...
s16 *sLevelDynamics[] = {
    ...
    _, _, _, _, _, _, _, _, _, _, sDynDdd, _, _, _, _, _, _, _, _, _, _, _,
    _, _, _, sDynUnk38
};
...
struct MusicDynamic sMusicDynamics[8] = {
    {0x0000, 0, 127, 100, 0x0e43, 0, 0, 100},
    {0x0003, 0, 127, 100, 0x0e40, 0, 0, 100},
    {0x0e43, 0, 127, 200, 0x0000, 0, 0, 200},
    {0x02ff, 0, 127, 100, 0x0100, 0, 0, 100},
    {0x03f7, 0, 127, 100, 0x0008, 0, 0, 100},
    {0x0070, 0, 127, 10,  0x0000, 0, 0, 100},
    {0x0000, 0, 127, 100, 0x0070, 0, 0, 10},
    {0xffff, 0, 127, 100, 0x0000, 0, 0, 100}
};
...
struct SoundCharacteristics gSoundBanks[SOUND_BANK_COUNT][40]; // list item memory for D_803320A4 and D_803320B0
...
static void func_8031DFE8(u8 bankIndex, u8 item)
{
    ...
    gSoundBanks[bankIndex][gSoundBanks[bankIndex][item].prev].next =
        gSoundBanks[bankIndex][item].next;
    ...
    while (soundIndex != 0xff)
    {
        ...
        if (gSoundBanks[bankIndex][soundIndex].soundStatus != SOUND_STATUS_STOPPED && soundIndex == spDB)
        {
            gSoundBanks[bankIndex][soundIndex].distance = sqrtf(
                    (*gSoundBanks[bankIndex][soundIndex].x * *gSoundBanks[bankIndex][soundIndex].x) +
                    (*gSoundBanks[bankIndex][soundIndex].y * *gSoundBanks[bankIndex][soundIndex].y) +
                    (*gSoundBanks[bankIndex][soundIndex].z * *gSoundBanks[bankIndex][soundIndex].z)) * 1;
    ...
    for (i = 0; i < D_80360C28[bankIndex]; i++)
    {
        ...
        if (soundIndex != 0xff)
        {
            if (D_80360C38[bankIndex][i] != 0xff)
            {
                ...
                {
                    if (gSoundBanks[bankIndex][D_80360C38[bankIndex][i]].soundStatus == SOUND_STATUS_PLAYING)
                    {
                ...
                else
                {
                    if (val2 == SOUND_STATUS_PLAYING &&
                        gSoundBanks[bankIndex][D_80360C38[bankIndex][i]].soundStatus != SOUND_STATUS_STOPPED)
                    {
                        gSoundBanks[bankIndex][D_80360C38[bankIndex][i]].soundStatus = SOUND_STATUS_STARTING;
                    }
                }
            }
            ...
            for (i = 0; i < D_80360C28[bankIndex]; i++)
            {
                if (D_80360C38[bankIndex][i] == 0xff)
                {
                    ...
                    gSoundBanks[bankIndex][sp88[soundIndex]].soundBits =
                        (gSoundBanks[bankIndex][sp88[soundIndex]].soundBits & ~SOUNDARGS_MASK_STATUS) + 1;
                    sp88[i] = 0xff;
                    i = 0xfe; // "break;"
                }
            }
...
static f32 get_sound_pan(f32 x, f32 z)
{
    ...
    else if (x >= US_FLOAT(0.0) && absX >= absZ)
    {
        // far right pan
        pan = US_FLOAT(1.0) -
            (US_FLOAT(44000.0) - absX) /
            (US_FLOAT(3.0) * (US_FLOAT(44000.0) - absZ));
    }
    else if (x < 0 && absX > absZ)
    {
        // far left pan
        pan = (US_FLOAT(44000.0) - absX) /
            (US_FLOAT(3.0) * (US_FLOAT(44000.0) - absZ));
    }
    else
    {
    ...
    for (bankIndex = 0; bankIndex < SOUND_BANK_COUNT; bankIndex++)
    {
        ...
        for (j = 0; j < 1; j++)
        {
            ...
            if (index < 0xff && gSoundBanks[bankIndex][index].soundStatus != SOUND_STATUS_STOPPED)
            {
                ...
                if (soundStatus == SOUND_STATUS_STARTING)
                {
                    ...
                    case 2:
                        ...
                        gSequencePlayers[2].channels[channelIndex]->pan = get_sound_pan(
                            *gSoundBanks[bankIndex][index].x,
                            *gSoundBanks[bankIndex][index].z);
                        gSequencePlayers[2].channels[channelIndex]->freqScale = get_sound_freq_scale(bankIndex, index);
                        gSequencePlayers[2].channels[channelIndex]->reverb = get_sound_reverb(bankIndex, index, channelIndex);
                        break;
                    case 3:
                    case 4:
                    case 5:
                ...
                else if (soundStatus == SOUND_STATUS_STOPPED && gSequencePlayers[2].channels[channelIndex]->layers[0]->finished == FALSE)
                {
                    func_8031E0E4(bankIndex, index);
                    gSequencePlayers[2].channels[channelIndex]->soundScriptIO[0] = 0;
                    func_8031DFE8(bankIndex, index);
...
void fade_channel_volume_scale(u8 player, u8 channelId, u8 targetScale, u16 fadeTimer)
{
    ...
    if (gSequencePlayers[player].channels[channelId] != &gSequenceChannelNone)
    {
        ...
        temp->velocity = ((f32) (targetScale / US_FLOAT(127.0)) - gSequencePlayers[player].channels[channelId]->volumeScale) / fadeTimer;
...
void func_8031F96C(u8 player)
{
    ...
    for (i = 0; i < CHANNELS_MAX; i++)
    {
        if (gSequencePlayers[player].channels[i] != &gSequenceChannelNone && D_80360928[player][i].remDuration != 0)
        {
            ...
            {
                gSequencePlayers[player].channels[i]->volumeScale = D_80360928[player][i].target / 127.0f;
            }
        }
    }
}
...
void unused_8031FED0(u8 player, u32 bits, s8 arg2)
{
    ...
    for (i = 0; i < CHANNELS_MAX; i++)
    {
        if (gSequencePlayers[player].channels[i] != &gSequenceChannelNone)
        {
            ...
            {
                gSequencePlayers[player].channels[i]->volumeScale = US_FLOAT(1.0) - (f32) arg2 / US_FLOAT(127.0);
            }
        }
        bits >>= 2;
    }
...
void func_803205E8(u32 soundBits, f32 *vec)
{
    ...
    while (item != 0xff)
    {
        if ((u16)(soundBits >> SOUNDARGS_SHIFT_SOUNDID) == (u16)(gSoundBanks[bankIndex][item].soundBits >> SOUNDARGS_SHIFT_SOUNDID) &&
...
void func_80320ED8(void)
{
    ...
    if (D_80363812 != 0 &&
            (D_80332120 == SEQ_EVENT_MERRY_GO_ROUND ||
             D_80332120 == SEQ_EVENT_PIRANHA_PLANT))
    {
