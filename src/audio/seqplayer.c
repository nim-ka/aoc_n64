                    ...
                    if (layer->portamento.mode != 0)
                    {
                        ...
                        if (layer->portamento.mode & 0x80)
                        {
                            portamento->speed = US_FLOAT(32512.0) * FLOAT_CAST(seqPlayer->tempo) /
                                ((f32) layer->delay * (f32) gTempoInternalToExternal * FLOAT_CAST(layer->portamentoTime));
                        }
                        else
                        {
        ...
        for (;;)
        {
            ...
            if (cmd > 0xc0)
            {
                ...
                case 0xc5: // set up dynamic table, dynamically
                    if (value != -1)
                    {
                        sp5A = (*seqChannel->dynTable)[value][1] + ((*seqChannel->dynTable)[value][0] << 8);
                        seqChannel->dynTable = (void *)(seqPlayer->seqData + sp5A);
                    }
                    break;
                ...
                case 0xda: // set adsr envelope
                    sp5A = m64_read_s16(state);
                    seqChannel->adsr.envelope = (struct AdsrEnvelope *)
                        (seqPlayer->seqData + sp5A);
                    break;
            ...
            else
            {
                ...
                case 0xb0: // set dynamic layer
                    ...
                    {
                        temp = (*seqChannel->dynTable)[value][0] + ((*seqChannel->dynTable)[value][1] << 8);
...
void sequence_player_process_sequence(struct SequencePlayer *seqPlayer)
{
    ...
    if (seqPlayer->bankDmaInProgress == TRUE)
    {
        ...
        if (seqPlayer->bankDmaRemaining == 0)
        {
            ...
            func_8031784C(seqPlayer->loadingBank,
        ...
        else
        {
            ...
            audio_dma_partial_copy_async(&seqPlayer->bankDmaCurrDevAddr,
                    ...
                    &seqPlayer->bankDmaMesgQueue, &seqPlayer->bankDmaIoMesg);
        }
        return;
    }
        ...
        for (;;)
        {
            ...
            if (cmd >= 0xc0)
            {
                ...
                case 0xdb: // set volume
                    ...
                    case SEQUENCE_PLAYER_STATE_2:
                        if (seqPlayer->fadeTimer != 0)
                        {
                            ...
                            seqPlayer->fadeVelocity = (targetVolume - seqPlayer->fadeVolume) / FLOAT_CAST(seqPlayer->fadeTimer);
                            break;
                        }
                        // fallthrough
                ...
                case 0xda: // increase/decrease volume
                    ...
                    seqPlayer->fadeVolume = seqPlayer->fadeVolume + (f32) tempSigned / US_FLOAT(127.0);
                    break;
