...
static void cutscene_take_cap_off(struct MarioState *m)
{
    ...
    SetSound(
        SOUND_ACTION_UNKNOWN43D << 16 | 0x8081,
...
static void cutscene_put_cap_on(struct MarioState *m)
{
    ...
    // SOUND_ACTION_UNKNOWN43E ?
    SetSound(
        SOUND_ACTION_UNKNOWN43E << 16 | 0x8081,
        &m->marioObj->header.gfx.unk54
    );
}
...
static s32 act_reading_npc_dialogue(struct MarioState *m)
{
    ...
    if (m->actionState < 8)
    {
        ...
        func_802507E8(
            m, m->heldObj == NULL ? \
            M_ANIM_DIALOGUE_FIRST_PERSON : M_ANIM_IDLE_WITH_LIGHT_OBJ
        );
    }
...
static s32 act_reading_automatic_dialogue(struct MarioState *m)
{
    ...
    if (m->actionState < 9)
    {
        func_802507E8(
            ...
            M_ANIM_WATER_IDLE : M_ANIM_DIALOGUE_FIRST_PERSON
        );
...
static s32 act_reading_sign(struct MarioState *m)
{
    ...
    func_80250F50(m, SOUND_ACTION_UNKNOWN45B << 16 | 0xFF81, MARIO_UNKNOWN_16);
    ...
    switch (m->actionState)
    {
        ...
        case 0:
            ...
            enable_time_stop();
            // reading sign
            func_802507E8(m, M_ANIM_DIALOGUE_FIRST_PERSON);
            m->actionState = 1;
...
static void general_star_dance_handler(struct MarioState *m, s32 isInWater)
{
    ...
    if (m->actionState == 0)
    {
        ...
        {
            case 1:
                spawn_object(m->marioObj, MODEL_STAR, beh_celebration_star);
                func_80248D48();
                if (m->actionArg & 1)
                {
                    func_803211B0();
...
static s32 act_fall_after_star_grab(struct MarioState *m)
{
    ...
    func_802507E8(m, M_ANIM_GENERAL_FALL);
    return FALSE;
}
...
{
    s32 animFrame = func_802507E8(m, animation);
...
static s32 act_standing_death(struct MarioState *m)
{
    ...
    func_80250F50(m, SOUND_MARIO_DYING << 16 | 0xFF81, MARIO_UNKNOWN_16);
    common_death_handler(m, M_ANIM_DYING_FALL_OVER, 80);
    if (m->marioObj->header.gfx.unk38.animFrame == 77)
        func_80251280(m, SOUND_ACTION_UNKNOWN418);
    return FALSE;
}
...
{
    func_80250F50(m, SOUND_MARIO_DYING << 16 | 0xFF81, MARIO_UNKNOWN_16);
    common_death_handler(m, M_ANIM_ELECTROCUTION, 43);
    return FALSE;
}
...
{
    func_80250F50(m, SOUND_MARIO_DYING << 16 | 0xFF81, MARIO_UNKNOWN_16);
    common_death_handler(m, M_ANIM_SUFFOCATING, 86);
    return FALSE;
}
...
{
    func_80250F50(m, SOUND_MARIO_DYING << 16 | 0xFF81, MARIO_UNKNOWN_16);
    if (common_death_handler(m, M_ANIM_DYING_ON_BACK, 54) == 40)
...
{
    func_80250F50(m, SOUND_MARIO_DYING << 16 | 0xFF81, MARIO_UNKNOWN_16);
    if (common_death_handler(m, M_ANIM_DYING_ON_STOMACH, 37) == 37)
        func_80251348(m, 0x04188081);
    return FALSE;
}
...
static s32 act_quicksand_death(struct MarioState *m)
{
    if (m->actionState == 0)
    {
        func_802507E8(m, M_ANIM_DYING_IN_QUICKSAND);
        func_80250AAC(m, 60);
        m->actionState = 1;
    }
    if (m->actionState == 1)
    ...
    SetSound(SOUND_UNKNOWN_UNK1414, &m->marioObj->header.gfx.unk54);
    return FALSE;
}

static s32 act_eaten_by_bubba(struct MarioState *m)
{
    func_80250F50(m, 0x2415FF81, MARIO_UNKNOWN_16);
    func_802507E8(m, M_ANIM_A_POSE);
    m->marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_01;
    m->health = 0xFF;
    if (m->actionTimer++ == 60)
...
static s32 act_unlocking_key_door(struct MarioState *m)
{
    ...
    if (m->actionTimer == 0)
    {
        spawn_obj_at_mario_rel_yaw(m, MODEL_BOWSER_KEY, beh_door_key1, 0);
        func_802507E8(m, M_ANIM_UNLOCK_DOOR);
    }
    ...
    switch (m->marioObj->header.gfx.unk38.animFrame)
    {
        case 79:
            SetSound(
                SOUND_GENERAL_SWITCH4 << 16 | 0x0081,
                &m->marioObj->header.gfx.unk54
            );
            break;
        case 111:
            SetSound(
                SOUND_GENERAL_SWITCH2 << 16 | 0x0081,
                &m->marioObj->header.gfx.unk54
            );
            break;
    }
...
static s32 act_unlocking_star_door(struct MarioState *m)
{
    switch (m->actionState)
    {
        case 0:
            ...
            func_802507E8(m, M_ANIM_SUMMON_STAR);
            m->actionState++;
            break;
        case 1:
            if (func_80250770(m))
            {
                spawn_object(m->marioObj, MODEL_STAR, beh_unlock_door_star);
                m->actionState++;
            }
            break;
        case 2:
            if (m->actionTimer++ == 70)
            {
                func_802507E8(m, M_ANIM_RETURN_STAR_APPROACH_DOOR);
                m->actionState++;
            }
            break;
        case 3:
...
static s32 act_going_through_door(struct MarioState *m)
{
    if (m->actionTimer == 0)
    {
        ...
        {
            m->interactObj->oInteractStatus = 0x00010000;
            func_802507E8(m, M_ANIM_PULL_DOOR_WALK_IN);
        }
        else
        {
            m->interactObj->oInteractStatus = 0x00020000;
            func_802507E8(m, M_ANIM_PUSH_DOOR_WALK_IN);
        }
    }
    m->faceAngle[1] = m->usedObj->oMoveAngleYaw;
...
static s32 act_emerge_from_pipe(struct MarioState *m)
{
    ...
    {
        if (gCurrAreaIndex == 2)
            func_80250F50(m, 0x7017A081, MARIO_UNKNOWN_16);
        else
            func_80250F50(m, 0x7016A081, MARIO_UNKNOWN_16);
    }

    if (launch_mario_until_land(m, ACT_JUMP_LAND_STOP, M_ANIM_SINGLE_JUMP, 8.0f))
    {
...
static s32 act_spawn_spin_airborne(struct MarioState *m)
{
    ...
    {
        if (func_802507E8(m, M_ANIM_FORWARD_SPINNING) == 0) // first anim frame
            SetSound(SOUND_ACTION_SWISH2, &m->marioObj->header.gfx.unk54);
    }
...
{
    stop_and_set_height_to_floor(m);
    func_802507E8(m, M_ANIM_GENERAL_LAND);
    if (func_80250770(m))
    {
        func_8024980C(0);
        set_mario_action(m, ACT_IDLE, 0);
...
static s32 act_exit_airborne(struct MarioState *m)
{
    if (15 < m->actionTimer++ && \
        launch_mario_until_land(
            m, ACT_EXIT_LAND_SAVE_DIALOGUE, M_ANIM_GENERAL_FALL, -32.0f
        )
    )
        // heal mario
        m->healCounter = 31;
...
static s32 act_falling_exit_airborne(struct MarioState *m)
{
    if (
        launch_mario_until_land(
            m, ACT_EXIT_LAND_SAVE_DIALOGUE, M_ANIM_GENERAL_FALL, 0.0f
        )
    )
        // heal mario
        m->healCounter = 31;
...
static s32 act_exit_land_save_dialogue(struct MarioState *m)
{
    ...
    switch (m->actionState)
    {
        ...
        case 0:
            func_802507E8(
                m, m->actionArg == 0 ? \
                M_ANIM_GENERAL_LAND : M_ANIM_LAND_FROM_SINGLE_JUMP
            );
            if (func_802507AC(m))
            {
        ...
        // key exit
        case 1:
            animFrame = func_802507E8(m, M_ANIM_THROW_CATCH_KEY);
            switch (animFrame)
            {
                case -1:
                    spawn_obj_at_mario_rel_yaw(m, 200, beh_door_key2, -32768);
                    //! fall through
                case 67:
                    SetSound(SOUND_ACTION_SWISH1 << 16 | 0x8081, &m->marioObj->header.gfx.unk54);
                    //! fall through
                case 83:
                    SetSound(SOUND_ACTION_PATBACK << 16 | 0x8081, &m->marioObj->header.gfx.unk54);
                    //! fall through
                case 111:
                    SetSound(SOUND_ACTION_UNKNOWN45C << 16 | 0x8081, &m->marioObj->header.gfx.unk54);
                    // no break
            }
            handle_save_menu(m);
            break;
        // exit without cap
        case 2:
            animFrame = func_802507E8(m, M_ANIM_MISSING_CAP);
        ...
        // exit with cap
        case 3:
            animFrame = func_802507E8(m, M_ANIM_TAKE_CAP_OFF_THEN_ON);
            switch (animFrame)
            {
                case 12:
                ...
                // fall through
                case 53:
                    SetSound(SOUND_ACTION_BRUSHHAIR << 16 | 0x8081, &m->marioObj->header.gfx.unk54);
                    break;
                case 82:
...
static s32 act_death_exit(struct MarioState *m)
{
    if (15 < m->actionTimer++ && \
        launch_mario_until_land(
            m, ACT_DEATH_EXIT_LAND, M_ANIM_GENERAL_FALL, -32.0f
        )
    )
    {
        #ifdef VERSION_JP
...
static s32 act_unused_death_exit(struct MarioState *m)
{
    if (
        launch_mario_until_land(
            m, ACT_FREEFALL_LAND_STOP, M_ANIM_GENERAL_FALL, 0.0f
        )
    )
    {
        #ifdef VERSION_JP
...
static s32 act_falling_death_exit(struct MarioState *m)
{
    if (
        launch_mario_until_land(
            m, ACT_DEATH_EXIT_LAND, M_ANIM_GENERAL_FALL, 0.0f
        )
    )
    {
        #ifdef VERSION_JP
...
static s32 act_special_exit_airborne(struct MarioState *m)
{
    ...
    if (
        launch_mario_until_land(
            m, ACT_EXIT_LAND_SAVE_DIALOGUE, M_ANIM_SINGLE_JUMP, -24.0f
        )
    )
    {
        // heal mario
...
{
    launch_mario_until_land(
        m, ACT_SPAWN_NO_SPIN_LANDING, M_ANIM_GENERAL_FALL, 0.0f
    );
    if (m->pos[1] < m->waterLevel - 100)
        func_8025325C(m);
    return FALSE;
...
static s32 act_bbh_enter_spin(struct MarioState *m)
{
    ...
    switch (m->actionState)
    {
        ...
        case 1:
            ...
            mario_set_forward_vel(m, forwardVel);
            if (func_802507E8(m, M_ANIM_FORWARD_SPINNING) == 0)
                SetSound(SOUND_ACTION_SWISH2, &m->marioObj->header.gfx.unk54);
        ...
        case 3:
            ...
            {
                func_80250F50(m, 0x0446A081, MARIO_UNKNOWN_16);
                func_802507E8(m, M_ANIM_DIVE);
...
static s32 act_bbh_enter_jump(struct MarioState *m)
{
    ...
    func_80251218(m, m->flags & MARIO_METAL_CAP ? 0x04289081 : 0x04008081, 1);
    func_80250FBC(m);
    ...
    func_802507E8(m, M_ANIM_DOUBLE_JUMP_RISE);
    perform_air_step(m, 0);
...
{
    func_80250F50(m, 0x0457C081, MARIO_UNKNOWN_16);
    func_802507E8(
        m, m->prevAction == ACT_CROUCHING ? \
        M_ANIM_CROUCHING : M_ANIM_DIALOGUE_FIRST_PERSON
    );
...
static s32 act_shocked(struct MarioState *m)
{
    ...
    if (func_802507E8(m, M_ANIM_SHOCKED) == 0)
    {
        m->actionTimer++;
...
{
    s32 animFrame = func_802507E8(m, M_ANIM_PUT_CAP_ON);
...
)
{
    ...
    // someone keeps excluding these defines in audio_defines.h >:(
    if (animFrame == -1)
        func_802510E4(m, 0x04488081, 1);

    else if (animFrame == unstuckFrame)
        func_802510E4(m, 0x04438081, 1);
...
static s32 act_head_stuck_in_ground(struct MarioState *m)
{
    stuck_in_ground_handler(
        m, M_ANIM_HEAD_STUCK_IN_GROUND, 96, 105, 135, ACT_IDLE
    );
    return FALSE;
}
...
{
    stuck_in_ground_handler(
        m, M_ANIM_BOTTOM_STUCK_IN_GROUND, 127, 136, -2, ACT_GROUND_POUND_LAND
    );
    return FALSE;
}
...
{
    stuck_in_ground_handler(
        m, M_ANIM_LEGS_STUCK_IN_GROUND, 116, 129, -2, ACT_IDLE
    );
    return FALSE;
}
...
static void intro_cutscene_raise_pipe(struct MarioState *m)
{
    ...
    if (m->actionTimer == 0)
        SetSound(SOUND_MENU_EXITPIPE << 16 | 0xA081, &sIntroWarpPipeObj->header.gfx.unk54);
...
static void intro_cutscene_jump_out_of_pipe(struct MarioState *m)
{
    ...
    if (m->actionTimer++ >= 118)
    {
        ...
        #ifndef VERSION_JP
        func_80250F50(m, 0x0444A081, MARIO_UNKNOWN_16);
        #endif
        ...
        if (perform_air_step(m, 0) == AIR_STEP_LANDED)
        {
            ...
            func_80251280(m, SOUND_ACTION_UNKNOWN408);
            #ifndef VERSION_JP
            SetSound(SOUND_MARIO_HAHA, &m->marioObj->header.gfx.unk54);
            #endif
            advance_cutscene_step(m);
        }
    }
...
{
    func_802507E8(m, M_ANIM_LAND_FROM_SINGLE_JUMP);
...
static void intro_cutscene_lower_pipe(struct MarioState *m)
{
    if (m->actionTimer++ == 0)
    {
        SetSound(SOUND_MENU_ENTERPIPE, &sIntroWarpPipeObj->header.gfx.unk54);
        func_802507E8(m, M_ANIM_DIALOGUE_FIRST_PERSON);
    }
...
static s32 grand_star_cutscene_taking_off(struct MarioState *m)
{
    ...
    if (m->actionState == 0)
    {
        func_802507E8(m, M_ANIM_FINAL_BOWSER_RAISE_HAND_SPIN);
    ...
    {
        animFrame = func_802507E8(m, M_ANIM_FINAL_BOWSER_WING_CAP_TAKE_OFF);
        if (animFrame == 3 || animFrame == 28 || animFrame == 60)
            func_802510E4(m, 0x04008081, 1);
        if (animFrame >= 3)
        ...
        switch (animFrame)
        {
            ...
            case 60:
                SetSound(SOUND_MARIO_YAHOO, &m->marioObj->header.gfx.unk54);
                break;
        }
        m->particleFlags |= PARTICLE_SPARKLES;
...
static s32 grand_star_cutscene_flying(struct MarioState *m)
{
    ...
    {
        case 0:
            func_802507E8(m, M_ANIM_WING_CAP_FLY);
...
static void end_peach_cutscene_summon_grand_star(struct MarioState *m)
{
    func_802507E8(
        m, m->actionState == 0 ? \
        M_ANIM_CREDITS_RAISE_HAND : M_ANIM_CREDITS_LOWER_HAND
    );
    ...
    SetSound(0x600B4001, &sEndGrandStarObj->header.gfx.unk54);
}
...
static void end_peach_cutscene_spawn_peach(struct MarioState *m)
{
    ...
    if (m->actionTimer >= 40)
    {
        SetSound(0x600B4001, &sEndPeachObj->header.gfx.unk54);
    }
}
...
static void end_peach_cutscene_descend_peach(struct MarioState *m)
{
    ...
    else
    {
        if (m->actionState >= 27)
            m->actionState -= 2;
        func_802507E8(m, M_ANIM_CREDITS_RETURN_FROM_LOOK_UP);
    }
...
static void end_peach_cutscene_run_to_peach(struct MarioState *m)
{
    ...
    func_80250934(m, 114, 0x00080000);
...
{
    s32 animFrame = func_802507E8(
        m, m->actionState == 0 ? \
        M_ANIM_CREDITS_TAKE_OFF_CAP : M_ANIM_CREDITS_LOOK_UP
    );
    ...
    switch (m->actionTimer)
    {
        ...
        case 230:
            ...
            SetSound(
                SOUND_PEACH_MARIO << 16 | 0xFF81,
                &sEndPeachObj->header.gfx.unk54
            );
        #endif
            break;
        ...
        #ifndef VERSION_JP
            SetSound(
                SOUND_PEACH_POWEROFTHESTARS << 16 | 0xFF81,
                &sEndPeachObj->header.gfx.unk54
            );
        #endif
            break;
...
static void end_peach_cutscene_dialogue_2(struct MarioState *m)
{
    ...
    switch (m->actionTimer)
    {
        ...
        #ifndef VERSION_JP
            SetSound(
                SOUND_PEACH_THANKSTOYOU << 16 | 0xFF81,
                &sEndPeachObj->header.gfx.unk54
            );
        #endif
            break;
        ...
        #ifndef VERSION_JP
            SetSound(
                SOUND_PEACH_THANKYOUMARIO << 16 | 0xFF81,
                &sEndPeachObj->header.gfx.unk54
            );
        #endif
            break;
        ...
        #ifndef VERSION_JP
            SetSound(
                SOUND_PEACH_SOMETHINGSPECIAL << 16 | 0xFF81,
                &sEndPeachObj->header.gfx.unk54
            );
        #endif
            break;
...
static void end_peach_cutscene_kiss_from_peach(struct MarioState *m)
{
    ...
    if (m->actionTimer >= 90)
        m->unk98->unk05 = m->actionTimer < 110 ? \
            sMarioBlinkOverride[m->actionTimer - 90] : 2;
...
{
    s32 animFrame = func_802507E8(m, M_ANIM_CREDITS_PEACE_SIGN);
...
static void end_peach_cutscene_dialogue_3(struct MarioState *m)
{
    ...
    switch (m->actionTimer)
    {
        ...
        #ifndef VERSION_JP
            SetSound(SOUND_PEACH_BAKEACAKE << 16 | 0xFF81, &sEndPeachObj->header.gfx.unk54);
        #endif
            break;
        ...
        #ifndef VERSION_JP
            SetSound(SOUND_PEACH_FORMARIO << 16 | 0xFF81, &sEndPeachObj->header.gfx.unk54);
        #endif
            break;
    }
...
static void end_peach_cutscene_run_to_castle(struct MarioState *m)
{
    func_802507E8(
        m, m->actionState == 0 ? \
        M_ANIM_CREDITS_START_WALK_LOOK_UP : M_ANIM_CREDITS_LOOK_BACK_THEN_RUN
    );
    ...
    #ifndef VERSION_JP
        SetSound(SOUND_PEACH_MARIO2 << 16 | 0xFF81, &sEndPeachObj->header.gfx.unk54);
    #endif
    }
...
static s32 act_end_waving_cutscene(struct MarioState *m)
{
    ...
    func_802507E8(m, M_ANIM_CREDITS_WAVING);
    stop_and_set_height_to_floor(m);
